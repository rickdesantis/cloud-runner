...
public static final double PRICE_MARGIN = 0.2;
	...
	public static VirtualMachine getVM(String name) throws CloudException {
		try {
			...
			throw new CloudException("Error while loading the configuration.", e);
		}
		throw new CloudException("VM not found in the configuration file!");
	}
	
	public VirtualMachine(String name, String ami, int instances, String size, int diskSize, double maxPrice, String os, String keyName, String sshUser, String sshPassword, Map<String, String> otherParams) throws CloudException {
		if (Configuration.AWS_CREDENTIALS == null)
			throw new CloudException("You didn't provide a valid credentials file, aborting.");
		
		if (instances <= 0 || diskSize <= 0)
			throw new CloudException("There's some error in your configuration, aborting.");
